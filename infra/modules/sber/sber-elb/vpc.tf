...
resource "sbercloud_vpc_subnet" "this" {
  for_each = try({for v in var.subnets : v.name => v}, {})
...
resource "sbercloud_nat_gateway" "this" {
  ...
  subnet_id             = lookup(sbercloud_vpc_subnet.this[each.value.name], "id")
  vpc_id                = local.vpc_id
  spec                  = lookup(each.value.nat_gw, "spec")
  description           = "For ${each.value.name}"
...
resource "sbercloud_vpc_eip" "snat" {
  ...
  bandwidth {
    ...
    name        = "${each.value.name}-nat-gw-eip"
...
resource "sbercloud_nat_snat_rule" "this" {
  ...
  subnet_id      = sbercloud_vpc_subnet.this[each.value.name].id
...
resource "sbercloud_vpc_route_table" "subnet" {
  ...
  name     = "${each.value.name}-rtb"
  vpc_id   = local.vpc_id
  subnets  = [sbercloud_vpc_subnet.this[each.value.name].id]
