...
output "subnet_ids" {
  ...
  value       = flatten([for subnet in sbercloud_vpc_subnet.this : subnet.id])
}

output "subnet_cidr_blocks" {
  description = "List of cidrs of subnets"
  value       = flatten([for subnet in sbercloud_vpc_subnet.this : subnet.cidr])
}

output "vpc_eip" {
  description = "List of cidrs of subnets"
  value       = sbercloud_vpc_eip.snat[0].address
}
